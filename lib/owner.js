const { getSetting, setSetting } = require('../database/store');
  ...
  module.exports = async (sock, msg, command, args, isOwner) => {
    ...
    if (command === 'addowner') {
      ...
      if (!owners.includes(num)) { owners.push(num); setSetting('owners', owners); }
    ...
    if (command === 'removeowner') {
      ...
      setSetting('owners', owners.filter(o => o !== num));
    ...
    if (command === 'pair') {
      if (!text) return sock.sendMessage(from, { text: '❌ Usage: .pair <number>' }, { quoted: msg }), true;
      const num = text.replace(/[^0-9]/g, '');
      try {
        const code = await sock.requestPairingCode(num);
        await sock.sendMessage(from, { text: `🔗 *Pairing Code for ${num}:*\n\n*${code}*\n\n_Open WhatsApp → Linked Devices → Link with phone number_` }, { quoted: msg });
      } catch (e) {
