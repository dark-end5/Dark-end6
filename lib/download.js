...
async function downloadWithYtdlCore(url, format) {
  ...
  const tmpFile = path.join(os.tmpdir(), `${Date.now()}_lesta.${ext}`);
  return new Promise((resolve, reject) => {
    ...
    stream.pipe(file);
    file.on('finish', () => resolve({ path: tmpFile, title }));
  ...
  try {
    if (command === 'play' || command === 'ytmp3') {
      ...
      if (!text.includes('youtube.com') && !text.includes('youtu.be')) {
        ...
        if (!url) throw new Error('No search results found');
      }
      ...
      if (useYtdlp) {
        ...
        if (!result) throw new Error('ytdl-core unavailable. Install yt-dlp for downloads on Termux:\npip install yt-dlp');
      }
      ...
      if (useYtdlp) {
        ...
        if (!result) throw new Error('ytdl-core unavailable. Install yt-dlp:\npip install yt-dlp');
      }
      const buffer = await fs.readFile(result.path);
    ...
    await sock.sendMessage(from, {
      text: `❌ Download failed: ${err.message}\n\n💡 Tip (Termux): _pkg install python && pip install yt-dlp_`
    }, { quoted: msg });
  }
  return true;
};
